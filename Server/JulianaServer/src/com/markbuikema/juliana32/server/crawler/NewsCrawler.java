...
public class NewsCrawler extends Thread {
	...
	private void processTeaserContent(String content) {
		if (!content.isEmpty()) {
			...
			String teasers = content.split("div id='teasers'>")[1].split("<div id='content-news-home'>")[0];
	...
	public TeaserNewsItem processTeaserNewsItem(String html) {
		...
		try {
			...
			String subTitle = html.replaceAll("<P>", "<p>").replaceAll("</P>", "</p>").split("<p>")[1].split("</p>")[0];
			...
			TeaserNewsItem item = new TeaserNewsItem(title, subTitle, imgUrl, detailUrl);
