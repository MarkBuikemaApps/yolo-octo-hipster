	...
	static PullToRefreshAttacher.ViewDelegate getBuiltInViewDelegate(final View view) {
		...
		for (final Map.Entry<Class, Class> entry : entries)
			if (entry.getKey().isInstance(view))
				return InstanceCreationUtils.newInstance(view.getContext(), entry.getValue(), VIEW_DELEGATE_CONSTRUCTOR_SIGNATURE,
						null);

		// Default is the ScrollYDelegate
		return InstanceCreationUtils.newInstance(view.getContext(), ScrollYDelegate.class, VIEW_DELEGATE_CONSTRUCTOR_SIGNATURE,
				null);
	}
