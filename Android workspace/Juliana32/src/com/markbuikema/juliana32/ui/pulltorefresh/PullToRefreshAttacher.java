...
public class PullToRefreshAttacher implements View.OnTouchListener {
	...
	protected PullToRefreshAttacher(Activity activity, Options options) {
		...
		mHeaderView = LayoutInflater.from(mEnvironmentDelegate.getContextForInflater(activity)).inflate(options.headerLayout,
				null);
		if (mHeaderView == null)
	...
	/**
	 * Manually set this Attacher's refreshing state. The header will be displayed
	 * or hidden as requested.
	 * 
	 * @param refreshing
	...
	/**
	 * Set a {@link HeaderViewListener} which is called when the visibility state
	 * of the Header View has changed.
	 * 
	 * @param listener
	 */
	...
	/**
	 * @return The Header View which is displayed when the user is pulling, or we
	 *         are refreshing.
	 */
	...
	public final boolean onTouch(final View view, final MotionEvent event) {
		// Just call onTouchEvent. It now handles the proper calling of
	...
	final boolean onInterceptTouchEvent(View view, MotionEvent event) {
		...
		if (DEBUG)
			Log.d(LOG_TAG, "onInterceptTouchEvent. Got ViewParams. " + view.toString());
		...
		case MotionEvent.ACTION_DOWN: {
			...
			if (canRefresh(true, params.onRefreshListener) && params.viewDelegate.isReadyForPull(view, event.getX(), event.getY())
					&& isReadyForPull())
	...
	final boolean onTouchEvent(View view, MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (mIsBeingDragged && y != mLastMotionY) {
				...
				/**
				 * Check to see if the user is scrolling the right direction (down). We
				 * allow a small scroll up which is the check against negative touch
				 * slop.
				 */
				if (yDx >= -mTouchSlop) {
					onPull(view, y);
	...
	public interface HeaderViewListener {
		...
		/**
		 * The state when the header view is minimized. By default this means that
		 * the progress bar is still visible, but the rest of the view is hidden,
		 * showing the Action Bar behind.
		 * <p/>
	...
	/**
	 * HeaderTransformers are what controls and update the Header View to reflect
	 * the current state of the pull-to-refresh interaction. They are responsible
	 * for showing and hiding the header view, as well as update the state.
	 */
	public static abstract class HeaderTransformer {
		...
		/**
		 * Called when the header should be reset. You should update any child views
		 * to reflect this.
		 * <p/>
		 ...
		 *          value between 0.0f and 1.0f depending on how far the user has
		 *          pulled.
		 */
		...
		/**
		 * Called when a refresh has begun. Theoretically this call is similar to
		 * that provided from {@link OnRefreshListener} but is more suitable for
		 * header view updates.
		 */
		...
		/**
		 * Called when the current refresh has taken longer than the time specified
		 * in {@link Options#refreshMinimizeDelay}.
		 */
		public void onRefreshMinimized() {
		}

		/**
		 * Called when the Header View should be made visible, usually with an
		 * animation.
		 * 
		...
		/**
		 * Called when the Header View should be made invisible, usually with an
		 * animation.
		 * 
		 * @return true if the visibility has changed.
		 */
	...
	/**
	 * ViewDelegates are what are used to de-couple the Attacher from the
	 * different types of scrollable views.
	 */
	public static abstract class ViewDelegate {

		/**
		 * Allows you to provide support for View which do not have built-in
		 * support. In this method you should cast <code>view</code> to it's native
		 * class, and check if it is scrolled to the top.
		 * 
		 * @param view
		 *          The view which has should be checked against.
		 * @param x
		 *          The X co-ordinate of the touch event
		 * @param y
		 *          The Y co-ordinate of the touch event
	...
	/**
	 * Allows you to specify a number of configuration options when instantiating
	 * a {@link PullToRefreshAttacher}. Used with {@link #get(Activity, Options)
	 * get()}.
	 */
	public static class Options {

		/**
		 * EnvironmentDelegate instance which will be used. If null, we will create
		 * an instance of the default class.
		 */
		public EnvironmentDelegate environmentDelegate = null;

		/**
		 * The layout resource ID which should be inflated to be displayed above the
		 * Action Bar
		 */
		public int headerLayout = DEFAULT_HEADER_LAYOUT;

		/**
		 * The header transformer to be used to transfer the header view. If null,
		 * an instance of {@link DefaultHeaderTransformer} will be used.
		 */
		public HeaderTransformer headerTransformer = null;

		/**
		 * The percentage of the refreshable view that needs to be scrolled before a
		 * refresh is initiated.
		 */
		public float refreshScrollDistance = DEFAULT_REFRESH_SCROLL_DISTANCE;

		/**
		 * Whether a refresh should only be initiated when the user has finished the
		 * touch event.
		 */
		 ...
		 * 'minimized'. By default, most of the header is faded out, leaving only
		 * the progress bar signifying that a refresh is taking place.
		 */
		public int refreshMinimizeDelay = DEFAULT_REFRESH_MINIMIZED_DELAY;

		/**
		 * Enable or disable the header 'minimization', which by default means that
		 * the majority of the header is hidden, leaving only the progress bar still
		 * showing.
		 * <p/>
		 * If set to true, the header will be minimized after the delay set in
		 * {@link #refreshMinimizeDelay}. If set to false then the whole header will
		 * be displayed until the refresh is finished.
		 */
		public boolean refreshMinimize = DEFAULT_REFRESH_MINIMIZE;
	}

	/**
	 * This class allows us to insert a layer in between the system decor view and
	 * the actual decor. (e.g. Action Bar views). This is needed so we can receive
	 * a call to fitSystemWindows(Rect) so we can adjust the header view to fit
	 * the system windows too.
	 */
	final static class DecorChildLayout extends FrameLayout {
		...
		DecorChildLayout(Context context, ViewGroup systemDecorView, View headerView) {
			...
			/**
			 * Wrap the Header View in a FrameLayout and add it to this view. It is
			 * wrapped so any inset changes do not affect the actual header view.
			 */
