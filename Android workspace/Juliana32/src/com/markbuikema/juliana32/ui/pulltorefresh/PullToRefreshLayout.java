 ...
 * FIXME
 */
public abstract class PullToRefreshLayout extends FrameLayout {
	...
	/**
	 * Set the {@link PullToRefreshAttacher} to be used with this layout. The view
	 * which is added to this layout will automatically be added as a
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (mPullToRefreshAttacher != null) {
			// This is an edge-case. If the ViewGroup does not contain a valid touch
			// target then
			// Android calls onTouchEvent after onInterceptTouchEvent with ACTION_DOWN
			// event.
			// If that happens then we need to find the visible view and pass it to
			// the attacher as
			// usual.
	...
	private View getChildForTouchEvent(MotionEvent event) {
		...
		for (int z = getChildCount() - 1; z >= 0; z--) {
			child = getChildAt(z);
			if (child.isShown() && x >= child.getLeft() && x <= child.getRight() && y >= child.getTop() && y <= child.getBottom()) {
				if (DEBUG)
