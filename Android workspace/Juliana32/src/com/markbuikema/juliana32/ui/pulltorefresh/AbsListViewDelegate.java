...
public class AbsListViewDelegate extends PullToRefreshAttacher.ViewDelegate {
	...
	public boolean isReadyForPull(View view, final float x, final float y) {
		...
		// Then we have to check whether the fas scroller is enabled, and check
		// we're not starting
	...
	int getVerticalScrollbarPosition(AbsListView absListView) {
		return Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB ? CompatV11.getVerticalScrollbarPosition(absListView)
				: Compat.getVerticalScrollbarPosition(absListView);
	}

	boolean isFastScrollAlwaysVisible(AbsListView absListView) {
		return Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB ? CompatV11.isFastScrollAlwaysVisible(absListView)
